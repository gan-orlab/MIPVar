...
srun --mem=40G --time=2:0:0 --account=rrg-grouleau-ac --cpus-per-task=1 $SCRIPT_FOLDER/Processing.step03.annotateVariants.part2.sh $output_name"_GF25.vcf" $BASE_DIR
echo "STEP 4 START"
srun --mem=40G --time=2:0:0 --account=rrg-grouleau-ac --cpus-per-task=3 parallel 'echo "STEP 4 START DP" {1}; \
        bash {3}/Processing.step04.removeLowQualVariants_GF_GQ_DP_MISS10.sh {2}_GF25_annotated.vcf {1} ;' ::: 15 30 50 ::: $output_name ::: $SCRIPT_FOLDER
echo "STEP 5 START"
